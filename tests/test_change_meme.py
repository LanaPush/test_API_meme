...
def test_change_url_of_meme(authorisation, change_meme_endpoint, create_meme_id_then_delete_it, get_meme_endpoint):
...
@allure.feature('change meme')
def test_change_text_of_meme_with_invalid_id(authorisation, create_meme_id, delete_meme_endpoint, change_meme_endpoint):
    meme_id = create_meme_id
    delete_meme_endpoint.delete_meme(meme_id, authorisation.get_headers())
    local_data_for_update = data_for_update.copy()
...
@allure.feature('change meme')
def test_change_meme_with_foreign_token(authorisation, change_meme_endpoint, create_meme_id_then_delete_it):
    meme_id = create_meme_id_then_delete_it
    local_data_for_update = data_for_update.copy()
    local_data_for_update.update({'id': meme_id, 'text': 'new_text'})
    change_meme_endpoint.change_meme(meme_id, payload=local_data_for_update, headers=authorisation.get_foreign_token())
    change_meme_endpoint.check_status_code_is_403()
    change_meme_endpoint.check_response_text_forbidden()
